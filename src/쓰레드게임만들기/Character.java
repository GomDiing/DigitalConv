...
public class Character extends Unit implements GameAction{
     ...
     * ultra : 특수 기술에대한 공격력
     * hp : 체력
     * 캐릭터가 가지는 기본적인 능력치를 생성자를 통해서 결정함 (캐릭터간의 밸런스 맞춤)
     */

    public Character(String name, int pP, int mP, double pH, double mH, int ultra, int hp) {
        this.name = name; // this는 자신의 객체를 참조하는 참조변수
        this.pPower = pP;
        this.mPower = mP;
        this.pHit = pH;
        this.mHit = mH;
    ...
    @Override
    // 물리 공격력 : 물리적인 힘 * 물리적인 확률로 계산하는게 원칙이지만 코드를 간단하게하기 위해서
    ...
    public boolean setDamage(double damage) {
        if (hp > damage) {
            ...
            return false; // flase는 캐릭터가 살아있음을 의미함
        } else {
